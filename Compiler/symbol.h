...
struct Node_Id_1
{
  ...
  int getEntry() {
    return addr.point_process.entry;
  }
  int getLength();
...
public:
  ...
  int lengthCal(Index_4D index_arr);
  int addressReturn;
};
...
public:
  ...
  */
  ...
  Node(string name, Catalog cat, int length) {
	  attribute atrNode;
	  atrNode.attributeId = Node_Id_1(name, cat);
	  this->attributeNode = atrNode;
	  this->kindNode = 3;
  ...
  bool operator==(const Node& n) {
    if (this->kindNode == n.kindNode) {
      if (n.kindNode == 3) {
        return this->attributeNode.attributeId == n.attributeNode.attributeId;
      }
      else if (n.kindNode == 4) {
        return this->attributeNode.attributeInt == n.attributeNode.attributeInt;
      }
      else {
        return false;
      }
  ...
  bool operator<(const Node& n) const {
    ...
    else if (n.kindNode == 3) {
      return this->attributeNode.attributeId.name < n.attributeNode.attributeId.name;
    }
    else if (n.kindNode == 4) {
      return this->attributeNode.attributeInt < n.attributeNode.attributeInt;
    }
  ...
  int getEntry() {
    return attributeNode.attributeId.getEntry();
  }
  int getLength() {
    return length;
  }
  ...
  void initParamNode(Node_Param node_param) {
    kindNode = 8;
    attributeNode.attributeParam = node_param;
  }

  vector<Node_Param> getTableParam()
  {
    return attributeNode.attributeId.addr.point_process.vparam;
  }
  vector<Node> getParamNodeVector() {
	  vector<Node_Param> tableParam = getTableParam();
	  vector<Node> paramNodeVector;
	  for (int i = 0; i < (int)tableParam.size(); ++i)
	  {
		  paramNodeVector.push_back(Node(tableParam[i].pname, tableParam[i].pcat, tableParam[i].length));
	  }
  ...
  Node getNodeStruct()
  {
    Node node;
    node.kindNode = 3;
    node.attributeNode.attributeId = nodeStruct;
    return node;
  }
  bool isMember() {
	  if (kindNode == 3 && attributeNode.attributeId.cat == d) {
		  return true;
	  }
	  return false;
  }
