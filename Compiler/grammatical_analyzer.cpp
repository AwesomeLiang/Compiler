...
void Parser::mapInsert(tag tag, string str)
{
  mapTag[tag] = str;
}
...
Parser::Parser()
{
  ...
  mapInsert(AND, "&&");
  mapInsert(BREAK, "break");
  mapInsert(ELSE, "else");
  mapInsert(EQ, "==");
  mapInsert(FALSE, "false");
  mapInsert(GE, ">=");
  mapInsert(ID, "id");
  mapInsert(IF, "if");
  mapInsert(LE, "<=");
  ...
  mapInsert(INT, "int");
  mapInsert(OR, "||");
  mapInsert(DOUBLE,"double" );
  mapInsert(TRUE, "true");
  mapInsert(WHILE, "while" );
  mapInsert(NUM, "num");
  mapInsert(REAL, "real");
  mapInsert(BITAND, "&");
  mapInsert(BITOR, "|");
  mapInsert(NOT, "!");
  mapInsert(ASSIGN, "==");
  mapInsert(ERROR, "error");
  mapInsert(LSP, "(");
  mapInsert(RSP, ")");
  mapInsert(LMP, "[");
  mapInsert(RMP, "]");
  mapInsert(RETURN, "return");
  mapInsert(SEMICOLON, ";");
  mapInsert(COMMA, ",");
  mapInsert(LBP, "{");
  mapInsert(RBP, "}");
  mapInsert(CHAR, "char" );
  mapInsert(STRING, "string");
  mapInsert(ADD, "+");
  mapInsert(SUB, "-");
  mapInsert(INCLUDE,"include" );
  mapInsert(MUL,"*" );
  mapInsert(DIV,"/" );
  mapInsert(FINISH, "finish");
  mapInsert(STRUCT,"struct" );
  mapInsert(CALL,"function" );
  mapInsert(PARAM, "param");
...
void Parser::action()
{
  ...
  while (pos <= (int)line.size() - 1 && con == 1)
  {
    ...
    case 1000: // error
    {
      cout << "error in line:" << num_line[pos - 1] << "  ; may be you need input these word";
      index temp;
    
      int i = temp_index.state;
      cout << i << "\t";
      for (int j = 0; j < (int)character.size(); ++j)
      {
        ...
        switch (table_analyse[temp].V)
        {
        case 997:
          cout << mapTag[character[j]]<<" ";
          break;
        case 998:
          cout << mapTag[character[j]] <<" ";
          break;
