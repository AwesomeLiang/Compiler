...
Token Lexer::scan()
{
  ...
  */
  ...
  if (peek == '$')
  {
    ...
    insert_delimiter("$", tok, l, VOID); peek = ' '; return tok;
  ...
  case('/'):
    if (readch('/')) { while (peek != '\n')readch(); insert_delimiter("", tok, l, VOID); return tok; }
    else
    {
      string str;
      str += peek;
  ...
  if (peek == ':')
  {
    ...
    insert_delimiter(str, tok, l, COLON);
    peek = ' ';
    return tok;
  }
  if (peek == ',')
  ...
  if (isLetter(peek))
  {
    ...
    if (peek == '_')
    {
      cout << "_  can't be the first letter" << "       error in line:"<<num_line << endl;
      peek = ' ';
